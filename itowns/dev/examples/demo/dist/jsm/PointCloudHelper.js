...
import { Coordinates, CopcSource, CopcLayer, EntwinePointTileSource, EntwinePointTileLayer, PotreeSource, PotreeLayer, Potree2Source, Potree2Layer, PNTS_MODE, } from 'itowns';
export function getFormat(url, format) {
    if (format !== 'auto') {
        return format;
    }
    if (url.includes('.copc.laz')) {
        return 'copc';
    }
    if (url.includes('ept.json')) {
        return 'ept';
    }
    if (url.includes('cloud.js')) {
        return 'potree';
    }
...
//# sourceMappingURL=PointCloudHelper.js.map