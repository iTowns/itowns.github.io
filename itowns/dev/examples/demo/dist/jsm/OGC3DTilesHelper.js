...
function zoomToBox(view, tile, box) {
    const radius = Math.max(new Vector3().fromArray(box, 3).length(), new Vector3().fromArray(box, 6).length(), new Vector3().fromArray(box, 9).length());
...
function zoomToRegion(view, region) {
    const extent = new Extent('EPSG:4326', region[0] * MathUtils.RAD2DEG, // west
    region[2] * MathUtils.RAD2DEG, // east
    region[1] * MathUtils.RAD2DEG, // south
    region[3] * MathUtils.RAD2DEG);
    return CameraUtils.getCameraTransformOptionsFromExtent(view, view.camera3D, extent);
}
...
//# sourceMappingURL=OGC3DTilesHelper.js.map