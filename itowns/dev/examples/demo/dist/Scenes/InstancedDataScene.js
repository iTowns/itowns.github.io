...
export const InstancedDataScene = {
  title: 'Populate Your City Model',
  description: 'Add thousands of 3D objects efficiently using instancing. ' + 'Perfect for trees, street furniture, or any repeating elements.',
  placement: {
  ...
  event: function update(/* dt */
    ...
    if (InstancedDataScene.meshes.length) {
      ...
      InstancedDataScene.getItownsView().notifyChange(InstancedDataScene.getItownsView().camera3D, true);
    }
  },
  onCreate: async () => {
  ...
  onEnter: async () => {
    InstancedDataScene.getItownsView().addFrameRequester(itowns.MAIN_LOOP_EVENTS.BEFORE_RENDER, InstancedDataScene.event);
  },
  onExit: async () => {
    InstancedDataScene.getItownsView().removeFrameRequester(itowns.MAIN_LOOP_EVENTS.BEFORE_RENDER, InstancedDataScene.event);
  }
};