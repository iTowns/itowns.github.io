...
import { BIMLoader } from "../ModelLoaders/index.js";
...
export const CombinedDataScene = {
  title: 'Combine Your Data',
  description: 'Visualize multiple data sources together to create rich, layered maps.',
  placement: {
  ...
  onCreate: async () => {
    ...
    CombinedDataScene.layers.push(await Layers.WorldDTMFetcherLayer.getLayer());
    CombinedDataScene.layers.push(await Layers.IgnMntHighResFetcherLayer.getLayer());
    CombinedDataScene.layers.push(await Layers.FlatBuildingsLayer.getLayer());
    CombinedDataScene.layers.push(await Layers.ParksLayer.getLayer());
    CombinedDataScene.layers.push(await Layers.Buildings3dLayer.getLayer(scaleZ));
    ...
    const pointCloudLayer = await Layers.PointCloudLayer.getLayer(itownsView.referenceCrs);
    ...
    model.quaternion.setFromUnitVectors(new THREE.Vector3(0, 1, 0), coord.geodesicNormal);
    const rotation = [0, 110, 0];
    const eulerRot = new THREE.Euler(THREE.MathUtils.degToRad(rotation[2]), -THREE.MathUtils.degToRad(rotation[1]), -THREE.MathUtils.degToRad(rotation[0]), 'ZYX');
    model.quaternion.multiply(new THREE.Quaternion().setFromEuler(eulerRot));
