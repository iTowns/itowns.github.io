...
import { ImmersiveView } from "../Views/index.js";
...
export const ImmersiveViewScene = {
  title: 'Walk Your Data',
  description: 'Experience your data at ground level with first-person navigation. ' + 'Perfect for street-level visualization and immersive exploration.',
  placement: {
  ...
  onCreate: async () => {
    ...
    let resolvePanoReady = null;
    const panoReady = new Promise(res => {
      resolvePanoReady = res;
    });
    ...
    const wfsBuildingLayer = await Layers.BuildingsWFSLayer.getLayer(mesh => mesh.traverse(object => {
    ...
    // property undefined
    itownsView.controls.transformationPositionPickOnTheGround = position => {
  ...
  onEnter: async () => {
    ...
    itownsView.notifyChange(itownsView.camera3D);
    itownsView.addEventListener(itowns.GLOBE_VIEW_EVENTS.GLOBE_INITIALIZED, ImmersiveViewScene.event);
  },
  onExit: async () => {
    const itownsView = ImmersiveViewScene.getItownsView();
    itownsView.removeEventListener(itowns.GLOBE_VIEW_EVENTS.GLOBE_INITIALIZED, ImmersiveViewScene.event);
  }
};