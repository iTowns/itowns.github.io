...
import { View3D } from "../Views/index.js";
import { SceneRepository } from "../Repositories/index.js";
import * as Layers from "../Layers/index.js";
import * as Models from "../ModelLoaders/index.js";
import { Globe3dScene } from "../Scenes/index.js";
import { FeaturePickerService } from "../Services/index.js";
 ...
 */
export const updateUIForScene = scene => {
 ...
 */
const cameraWorldToCoordinates = (view, targetCrs) => {
 ...
 * @returns Promise<any>
 */
export const moveCameraTo = (view, placement, duration = Config.DURATION) => itowns.CameraUtils.animateCameraToLookAtTarget(view, view.camera3D, {
 ...
 */
export const transitionToScene = async (currentScene, nextScene) => {
  ...
  const controls = transitionView.controls;
  ...
  if (!(currentScene.view instanceof View3D)) {
    await moveCameraTo(transitionView, currentScene.placement, 0.1).catch(console.error);
  }

  // Compute current coordinate from camera position
  const camCoords = cameraWorldToCoordinates(transitionView, nextScene.placement.coord.crs);
  ...
  if (currentScene.placement.range < minRange && nextScene.placement.range < minRange && distance > Config.MIN_DISTANCE) {
    cameraPromise = moveCameraTo(transitionView, {
      coord: camCoords,
      range: minRange,
      tilt: 89.5
    }, Config.DURATION / 2) // half duration since there are two steps in this case
    .catch(console.error).then(() => moveCameraTo(transitionView, nextScene.placement, Config.DURATION / 2).catch(console.error));
  } else {
  ...
  const layerPromise = new Promise(resolve => {
    ...
    for (const layer of nextScene.layers) {
      layer.visible = true;
      if (FeaturePickerService.layers.find(l => l.id === layer.id) && nextView instanceof View3D) {
        FeaturePickerService.enable(nextView);
        featurePickingInScene = true;
      }
    }
    if (FeaturePickerService.container) {
      FeaturePickerService.container.style.display = featurePickingInScene ? 'block' : 'none';
    }
    resolve();
  });
 ...
 * @returns Promise<void>
 */
export const hardResetScene = async scene => {
  await scene.onExit?.();
  ...
  for (const layer of scene.layers) {
    layer.visible = true;
    if (FeaturePickerService.layers.find(l => l.id === layer.id) && view instanceof View3D) {
 ...
 * @returns Promise<void>
 */
export const resetScene = async scene => {
  ...
  try {
    ...
    const controls = iTownsView.controls;
